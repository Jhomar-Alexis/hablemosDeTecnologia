<template>
    <div class="flex flex-col p-10">
        <h1 class="text-black text-2xl md:text-4xl font-[900] mb-10 text-center">
            ¿Cuáles son los beneficios y riesgos <br> del uso de Internet en la sociedad actual?
        </h1>
        <div class="flex flex-col-reverse lg:flex-row-reverse items-center justify-center pb-4 lg:pb-0 lg:p-4 gap-4">
            <!-- Caja de texto -->
            <div class=" w-full md:w-1/2 lg:w-1/2 xl:w-1/3 bg-cyan-700 p-4 rounded-lg shadow-[0_0_12px_0] shadow-black">
                <p class="text-white lg:text-2xl">El Internet representa un universo infinito de oportunidades para la
                    educación, la cultura, la comunicación y el entretenimiento; sin embargo, un uso inadecuado puede
                    generar problemas y afectaciones importantes.
                </p>
            </div>
            <!-- Caja de imagen -->
            <div class="w-full lg:w-1/4 xl:w-1/6 md:w-1/2 rounded-lg shadow-[0_0_12px_0] shadow-black">
                <img src="~/public/images/25.jpeg"
                    class="w-full h-auto object-cover rounded-lg border-4 border-cyan-700" />
            </div>
        </div>
    </div>
</template>